  ...
  const chartData = sortedYears.map((year) => {
    ...
    Object.keys(data).forEach((institution) => {
      if (Array.isArray(data[institution])) {
        const institutionYearData = data[institution].find(
          (item) => item.year === year
        );
        yearData[institution] = institutionYearData
          ? institutionYearData.total
          : null;
      } else {
        console.warn(
          `data[${institution}] is not an array:`,
          data[institution]
        );
    ...
    <div className="card">
      <div className="card-header greyBackground">
        ...
        <small className="text-muted">
          {t("EnrollmentTrendsDescription1")}
          <span className="accentText">
            {t("EnrollmentTrendsDescription2")}
          </span>
        </small>
      </div>
